...
- `native_bridge.py`（ADR-145/146/151/152）：驗證 native ABI 與 KBar dtype/stride，並把
...
- `engine_router.py`（ADR-149）：原生指標產品路由的單一契約。`off` 不載入 C++；
  ...
  ADR-152 新增 24 種策略條件的 typed batch compiler／bridge，輸出逐根 readonly
  `uint8` 訊號；尚未接管 runtime、Intent、風控、成交或績效。
...
│   ├─ native_bridge.py     ABI、SQLite、批次指標／條件 shadow API (ADR-145～152)
│   └─ order_rules.py
├─ native/                 CMake/MSVC x64/pybind11 原生核心 (ADR-145～152)
│   ├─ include/stockbuild/  版本、KBar、SQLite、resampler、indicator／condition 介面
│   ├─ src/                 SQLite RAII、resampler、indicator 與 condition 實作
