...
2. **Qt 成為下一代主圖渲染技術**，完整歷史留在資料層，畫面只依 viewport
   建立可見幾何，平移／縮放不得重新建立全資料集圖元。
3. **結果等價優先於速度**：Python 現行測試與既有 ADR 所定義的語意是基準。
   C++ 未逐項通過差異測試前，不得成為預設路徑。
4. **漸進導入且可立即回退**：每個子系統都有獨立 feature flag、Python
   參考路徑與 shadow mode，不做一次性全面重寫。
5. **券商與交易安全邊界不變**：C++ 只輸出券商中立 `OrderIntent`；登入、
...
```text
券商 SDK / SQLite / 設定 / 金鑰                         Qt 主視圖
          Python I/O 層                                  │
              │  immutable NumPy arrays / batches        │ viewport arrays
              ▼                                          ▼
...
| 功能 | C++ 責任 | Python 保留責任 |
|---|---|---|
| K 線歷史 | 欄式陣列、聚合、缺值／排序檢查 | SQLite 單一 writer、券商／官方資料下載 |
...
| 自訂 Python 策略 | C++ 掌管事件迴圈、成交、風控與績效；Python callback 僅產生 decision。可轉換語法再編譯為原生 IR，不能靜默改寫不支援語法 |
...
「所有策略以 C++ 為核心」定義為：**資料時間軸、部位狀態、風控、成交、成本、
績效與稽核一律由 C++ 掌管**。任意 Python `on_bar(ctx)` 在第一階段仍可作為
訊號外掛，因為 Python 動態語法不能可靠地自動翻譯為 C++。後續提供受限且可驗證的
Strategy IR（指標、比較、交叉、狀態變數、OPEN/CLOSE），可轉換者完全在 C++
批次執行；不可轉換者明確標示「Python 訊號相容模式」，絕不假稱已原生化。
...
- 定義 Strategy IR 與安全 AST→IR 編譯器；支援指標、參數、state、比較、交叉、
  OPEN/CLOSE/HOLD。既有任意 Python 先走 C++ orchestration + Python signal callback。
...
3. **性質測試**：OHLC 合法性、時間排序、upsert 後冪等、成本不增加績效、
   不同帳戶狀態隔離、平行結果可重現。
4. **原生安全**：編譯警告視為錯誤、ASan/UBSan、邊界／空值／超大資料 fuzz、
   ABI 版本與相依 DLL 檢查。
5. **Shadow telemetry**：正式功能仍使用 Python 結果時，背景抽樣跑 C++ 並只記
   匿名化差異摘要；不得把 API Key、帳號或完整策略原始碼寫進日誌。
6. **實機門檻**：真實 SQLite 長歷史、Qt 顏色／hover／縮放、模擬券商登入、
   runner 節拍與中止反應只能由實機確認；任何真實下單不屬於效能驗證。

## 十一、feature flags 與回退
...
native/
├─ CMakeLists.txt
├─ include/stockbuild/     types、schema、indicators、strategy、backtest、screener
├─ src/                    C++ 實作
...
| 自訂 Python 無法原生化 | C++ orchestration + 明示 callback 模式；受限 IR 分階段覆蓋 |
...
3. 選股、選股回測、最佳化與即時量化 runner 都使用同一份 native 條件／狀態語意。
4. Qt 主圖完整取代 Matplotlib 主圖，十年歷史與大量分 K 實機效能達標。
5. Python 仍是唯一券商／金鑰／真正送單邊界，所有交易安全診斷保持通過。
6. Windows 乾淨機安裝、升級、native 載入失敗回退、版本不相容拒載均完成驗證。
7. `test_core`、`test_brokers`、全部 diag、crossref、py_compile、native suite、
